...
def main():
    ...
    P['Келли_%'] = [100 * kelly(r.p_cond, r.кэф, frac=0.25, cap=0.02) for r in P.itertuples()]
    ...
    block('2. РЫНОК НЕ ВОЗРАЖАЕТ',
          ['перевес по xG-модели, расхождение с Pinnacle в пределах погрешности подгонки',
           'острая линия не подтверждает, но и не опровергает'],
          T2, show_kelly=True)
    ...
    play = pd.concat([T1, T2])
    if not play.empty:
        tot = float(play['Келли_%'].sum())
        scale = min(1.0, 6.0 / tot) if tot > 0 else 1.0
        print('')
        print(f'Размер ставок (ярусы 1-2, четверть-Келли, потолок 6% банка на тур): '
              f'{tot*scale:.1f}%')
